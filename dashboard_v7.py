...
let chartCandles=[],lastState=null,lastCandleDate='',chartTf='1m',chartSymbol='QQQ.US',chartDate='',configSnapshot=null,i18n={default:'zh',languages:{en:{},zh:{}}},currentLang='zh',showSignals=true,autoRefresh=true,refreshTimer=null,selectedProbeKey='',selectedProbe=null,probeFilter='all',probeSort='time_desc',strategyFilter='',reviewPeriod='day';
...
function setupControls(){document.querySelectorAll('.tf-btn[data-tf]').forEach(btn=>{btn.addEventListener('click',()=>{document.querySelectorAll('.tf-btn[data-tf]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');chartTf=btn.dataset.tf||'1m';loadCandles()})});document.querySelectorAll('[data-review-period]').forEach(btn=>{btn.addEventListener('click',()=>{document.querySelectorAll('[data-review-period]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');reviewPeriod=btn.dataset.reviewPeriod||'day';loadReviewSummary()})});const select=$('chart-symbol');if(select){select.addEventListener('change',async()=>{chartSymbol=select.value||'QQQ.US';chartDate='';await loadCandleDates();loadCandles()})}const dateSelect=$('chart-date');if(dateSelect){dateSelect.addEventListener('change',()=>{chartDate=dateSelect.value||'';loadCandles()})}const filter=$('probe-filter');if(filter){filter.addEventListener('change',()=>{probeFilter=filter.value||'all';updateProbes((lastState&&lastState.signal_probes)||[])})}const sort=$('probe-sort');if(sort){sort.addEventListener('change',()=>{probeSort=sort.value||'time_desc';updateProbes((lastState&&lastState.signal_probes)||[])})}const clearStrategy=$('clear-strategy-filter');if(clearStrategy){clearStrategy.addEventListener('click',()=>{strategyFilter='';updateProbes((lastState&&lastState.signal_probes)||[])})}const sig=$('toggle-signals');if(sig){sig.addEventListener('click',()=>{showSignals=!showSignals;sig.classList.toggle('active',showSignals);drawChart()})}const refresh=$('toggle-refresh');if(refresh){refresh.addEventListener('click',()=>{autoRefresh=!autoRefresh;refresh.classList.toggle('active',autoRefresh);setRefreshTimer()})}const lang=$('lang-select');if(lang){lang.addEventListener('change',()=>{currentLang=lang.value||'zh';localStorage.setItem('dashboard_lang',currentLang);applyI18n()})}}
...
connect();setupControls();loadI18n();loadSymbols();loadConfig();loadCandles();loadReviewSummary();setRefreshTimer();window.addEventListener('resize',()=>drawChart());
</script>
</body>
</html>'''
